...
# 鼓励语 & 失败安慰语
ENCOURAGE_MESSAGES = ["✅ 感谢你的真实评价！你的反馈对其他狼友很有帮助～", "👍 评价成功！口碑库因为你又丰富了一点！", "❤️ 收到！你的评价已经记录，狼友们会感谢你。"]
FAIL_MESSAGES = ["😔 理由有点简短呢～至少12个字哦", "💡 内容太简单了，试试写写具体感受", "⚠️ 可能包含敏感词，请修改后再试"]
...
def add_evaluation(teacher: str, recommend: int, reason: str, user_id: int) -> dict:
    ...
    conn = get_connection()
    try:
        conn.execute("INSERT OR IGNORE INTO recs (teacher, recommend, reason, user_id) VALUES (?, ?, ?, ?)",
...
def get_teacher_detail(teacher: str):
    conn = get_connection()
    rows = conn.execute("SELECT recommend, reason, substr(time,1,10) FROM recs WHERE teacher = ? ORDER BY time DESC", (teacher,)).fetchall()
    conn.close()
    if not rows:
        return None
